...
export function ArithGeo(arr) {
  const isArtith = [];
  const isGeo = [];

  for (let i = 0; i < arr.length - 2; i++) {
    if (arr[i + 1] - arr[i] === arr[i + 2] - arr[i + 1]) isArtith.push(1);
    else isArtith.push(0);

    if (arr[i + 1] / arr[i] === arr[i + 2] / arr[i + 1]) isGeo.push(1);
    else isGeo.push(0);
  }
  if (isArtith.includes(0) && !isGeo.includes(0)) return 'Geometric';
  else if (!isArtith.includes(0) && isGeo.includes(0)) return 'Arithmetic';
  else return -1;
}
